    ...
    if params[:update_planfact] == "1" && @branch
      @branch.groups.each do |group|
        group.employees.all.each do |employee|
          ...
          employee_planfact[:weight_plan] = employee.plan_weight(date) # Груз план
          employee_planfact[:weight_fact] = employee.fact_weight(date) # Груз факт
          employee_planfact[:income_plan] = employee.plan_incomes(date) # Поступления план
          employee_planfact[:income_fact] = employee.fact_incomes(date) # Поступления факт
          employee_planfact[:pro_percent] = employee.plan_percent(date) # % продления
        ...
        group_planfact[:weight_plan] = group.plan_weight(date)
        group_planfact[:weight_fact] = group.fact_weight(date)
        group_planfact[:income_plan] = group.plan_incomes(date)
        group_planfact[:income_fact] = group.fact_incomes(date)
        group_planfact[:pro_percent] = group.plan_percent(date)
      ...
      branch_planfact[:weight_plan] =  @branch.plan_weight(date)
      branch_planfact[:weight_fact] =  @branch.fact_weight(date)
      branch_planfact[:income_plan] = @branch.plan_incomes(date)
